...
export async function handleDonePacket(packet: InDonePacket, con: Connection, ws: WebSocket) {
	...
	if(TASK_VERIFICATION_NEEDED) {
		if(!awaitingVerification[course.uuid]?.find(v => v.uuid == con.uuid)?.verified) canContinue = false;
